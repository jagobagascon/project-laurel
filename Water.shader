...
uniform float FlowSpeed: hint_range(0.00, 10.0) = 1.0;
...
float get_noise(vec2 p, float t) {
	...
	if (Flows) {
		p.x += FlowSpeed * t * 10.0;
		t_detail.x *= FlowSpeed * -10.0;
	}
	float f = 0.0;
...
void fragment() {
	...
	if (negative_depth) {
		...
		// foam
		float foamMovement = FoamMovement ? sin(TIME * FoamSpeed) * .1 : 0.0;
		float foamCutoff = FoamCutoff + foamMovement;
		float shoreline_foam = noise_value < foamCutoff / 30.0 / water_depth ? 1.0 : 0.0;
